<template>
  <div class="configfieldbox" id="configfieldbox">
        ...
        <div class="tablebox">
          <el-table :data="tableData" height="400">
            ...
            <el-table-column prop="istitle" label="是否列表显示">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.istitle" active-text="" inactive-text="" @change="changetitle(scope.row)"> </el-switch>
              </template>
            </el-table-column>
            <el-table-column prop="columntype" label="数值类型">
              <template slot-scope="scope">
                <el-select v-model="scope.row.columntype" :placeholder="valDefault" :disabled="!scope.row.istitle">
                  <el-option v-for="item in valuestype" :key="item.key" :label="item.label" :value="item.key">
                  </el-option>
                </el-select>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="isquery" class="div" label="是否是查询条件">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isquery" active-text="" inactive-text="" @change="changequery(scope.row)"> </el-switch>
              </template>
            </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.valDefault = this.valuestype[0].label;
  ...
  methods: {
    ...
    GetTableColumnsByKey() {
      if (this.parentmsg === undefined) {
        ...
        getTableColumnsByKey(this.parentmsg.ids).then((res) => {
          // 因为不同的表下的字段的key 可能相同，所以需要手动定义一个唯一的key来作为是被标志，用来在回写数据的时候用到
          if (res.data !== undefined) {
            this.initfielddata = JSON.parse(JSON.stringify(res.data));
            for (let j in this.initfielddata) {
          ...
          if (this.parentmsg !== undefined) {
            if (this.parentmsg.fieldData.length > 0) {
              item = {
                ...
                allkey: item.allkey,
                queryvalue: '',
                columntype: this.valuestype[0].key
              }
            } else {
              // console.log('null', this.parentmsg);
            }
