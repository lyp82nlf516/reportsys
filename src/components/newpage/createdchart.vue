<template>
  <div class="createdchartbox" id="createdchartbox">
    <div class="allcon dragbox" id="dragbox" :style="{ height: divHeight  + 'px' }">
      <div class="dragdom querybox" v-for="(item, index) in querys" :key="item.key" :id="item.key" :class="('querys' + index)">
        <div class="content">
          ...
          <div class="inputdiv" v-if="item.type === 'input'">
            <div class="changewidthdiv" :id="('queryzoombox' + item.other)">
      ...
      <div class="dragdom textbox" v-for="(item, index) in text" :key="item.key" :id="item.key" :class="('text' + index)">
        <div class="content">
          <span class="textshow" :style="item.fontsize">{{item.data}}</span>
        </div>
        <div class="handle handlequery"></div>
      </div>
  ...
  methods: {
    ...
    GetDragComponent() {
      getDragComponent({
        ...
        if (res.status === 200) {
          // console.log('接口返回数据-----3333-：', res.data);
          this.delinitdata(res.data);
          if (res.data.charts.length > 0) {
            let data = res.data.charts;
            for (let i in data) {
              this.heightlist.push(data[i].position.top);
            }
          }
          ...
          this.divHeight = max + 100 + 250;
          // console.log('max-----3333-：', max, this.heightlist);
        }
      })
    },
    // 处理数据
    ...
    getchartsdata(res) {
      if (res.status === 200) {
        for (let i in res.data) {
          // console.log('设置图片的数据-res.data[i]:', res.data[i]);
          if (i.indexOf('charts') > -1) {
            ...
            // console.log('this.charts:', this.charts[temp].type);
            if (this.charts[temp].type === 'pie') {
              ...
              // console.log('this.charts----barline:', this.charts[temp].type);
              if (res.data[i] === null) {
                series = [{
                  type: 'line',
                  data: [],
