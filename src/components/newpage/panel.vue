      ...
      <div class="homePanel">
        <ul>
          <li
            ...
            :class="{'active': selected === item.privilegeMenuId}"
          ><span
              id="mainHome"
              @click="handleClick(item, index)"
              v-if="item.privilegeMenuId === '383'"
            > <img
            ...
            <span v-if="item.privilegeMenuId !== '383'">
              <span class="item" @click="handleClick(item, index)">{{ item.privilegeMenuName }}</span> <i
                class="el-icon-close"
                @click="removeTab(item.privilegeMenuId, item)"
              ></i></span></li>
        </ul>
      </div>
      <div class="hideMenu">
